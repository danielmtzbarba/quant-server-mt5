...
class MarketDataAPI:
    ...
    def get_historical_data(self, symbol: str, start: str, stop: str = "now()") -> pd.DataFrame:
        """
        ...
        try:
            ...
            columns_present = [col for col in ['Open', 'High', 'Low', 'Close', 'Volume'] if col in df.columns]
    ...
    def write_candles(self, symbol: str, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if points:
                ...
                logger.debug(f"Successfully wrote {len(points)} candle(s) for {symbol}.")
    ...
    def check_data_integrity(self, symbol: str, start: str, stop: str) -> dict:
        ...
        logger.info(f"Integrity Check - Rows: {result['total_rows']} | Duplicates: {result['duplicates']}")
        return result

    def get_resampled_candles(self, symbol: str, interval: str, start: str, stop: str = "now()") -> pd.DataFrame:
        """

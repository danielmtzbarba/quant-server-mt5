...
async def receive_signal(signal: TradingSignal):
    ...
    if not success:
        if reason == "MAX_POSITIONS_REACHED":
            raise HTTPException(status_code=429, detail="Maximum position limit reached. Trade gated.")
...
async def upload_candles(payload: TradeDBPayload):
    logger.info(f"POST /upload_candles: {payload.symbol} ({len(payload.candles)} candles)")
    result = await sync_db_service.upload_candles(
        payload.symbol, payload.timeframe, payload.gmt_offset, 
...
async def verify_history(payload: TradeDBPayload):
    ...
    return sync_db_service.verify_history(
        payload.symbol, payload.gmt_offset, 
        [c.model_dump() for c in payload.candles]
    )
...
async def dashboard(request: Request, symbol: str = "EURUSD", days: int = 3):
    ...
    if df is None or df.empty:
        return templates.TemplateResponse(
            request=request, name="dashboard.html", context={
                "symbol": symbol, "total_bars": 0, "last_price": "N/A", "last_sync": "N/A",
    ...
    fig = MarketVisualizer.get_figure(
        df=df, symbol=f"{symbol} (15M) Analysis", chart_type='candle', 
        overlays=['Sup_50', 'Res_50'], show_volume=True, highlights=highlights
    )
    chart_html = fig.to_html(full_html=False, include_plotlyjs='cdn')

    return templates.TemplateResponse(
        request=request, name="dashboard.html", context={
            "symbol": symbol, "total_bars": len(df), 

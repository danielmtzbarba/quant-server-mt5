...
class MarketVisualizer:
    ...
    def get_standard_highlights(df: pd.DataFrame, gaps: list = None) -> list:
        ...
        for fri in fridays:
            ...
            if sun_mask.any():
                ...
                first_sun_ts_ny = (fri + pd.Timedelta(days=2)).replace(hour=17, minute=0, second=0)
            ...
            highlights.append({
                'start': h_start, 
                'end': h_end,
                'color': '#FBC02D', 'label': 'WEEKEND', 'opacity': 0.15
            })
        ...
        for ts in df.index:
            ...
            if score == 10:
                ...
                if current_overlap_start is not None:
                    highlights.append({
                        'start': current_overlap_start, 'end': ts,
                        'color': '#26A69A', 'label': '', 'opacity': 0.1
                    })
                    current_overlap_start = None
        
        if current_overlap_start:
             highlights.append({
                 'start': current_overlap_start, 'end': df.index[-1], 
                 'color': '#26A69A', 'label': '', 'opacity': 0.1
             })
        ...
        if gaps:
            for g in gaps:
                highlights.append({
                    ...
                    'color': '#EF5350', 'label': 'GAP', 'opacity': 0.3
                })
    ...
    def plot_chart(
        ...
        fig = MarketVisualizer.get_figure(df, symbol, chart_type, overlays, show_volume, highlights)
        if fig:
    ...
    def get_figure(
            ...
            for i, col in enumerate(overlays):
                if col in df.columns:
                    fig.add_trace(go.Scatter(
                        x=df.index, y=df[col], 
                        mode='lines', name=col,
                        line=dict(width=1.5, color=INDICATOR_COLORS[i % len(INDICATOR_COLORS)])
                    ), row=1, col=1)
        ...
        if 'Signal' in df.columns:
            ...
            if not buys.empty:
                fig.add_trace(go.Scatter(
                    x=buys.index, 
                    y=buys['Low'] - (buys['Low'] * 0.0005), # Slightly below the wick
                    mode='markers', name='Buy Signal',
                    marker=dict(symbol='triangle-up', size=14, color='#00C853', line=dict(width=1, color='black'))
                ), row=1, col=1)

            if not sells.empty:
                fig.add_trace(go.Scatter(
                    x=sells.index, 
                    y=sells['High'] + (sells['High'] * 0.0005), # Slightly above the wick
                    mode='markers', name='Sell Signal',
                    marker=dict(symbol='triangle-down', size=14, color='#D50000', line=dict(width=1, color='black'))
                ), row=1, col=1)

        # --- 3. Volume Subplot ---
        if show_volume and 'Volume' in df.columns:
            # Match volume bars to candlestick colors
            marker_colors = [BULL_COLOR if close >= open else BEAR_COLOR 
                             for close, open in zip(df['Close'], df['Open'])]
            
            fig.add_trace(go.Bar(
                x=df.index, y=df['Volume'], 
                name='Volume', marker_color=marker_colors,
        ...
        fig.update_layout(
            ...
            xaxis=dict(range=[df.index.min(), df.index.max()], rangeslider_visible=False),

...
class DataHealthMonitor:
    @staticmethod
    def get_precision_forex_index(start_dt: datetime, end_dt: datetime) -> pd.DatetimeIndex:
        ...
        for ts in full_range:
            ...
            is_weekend = False
            if weekday == 5: is_weekend = True # Sat (Always)
            elif weekday == 4 and hour >= 17: is_weekend = True # Fri Night (Closed after 5 PM)
            elif weekday == 6 and hour < 17: is_weekend = True  # Sun morning (Closed before 5 PM)
    ...
    @classmethod
    def check_integrity(cls, symbol: str, days: int = 14, log_table: bool = True) -> dict:
        """
        ...
        with MarketDataAPI() as api:
            df = api.get_historical_data(symbol, start=f"-{days}d")
            
            if df.empty:
                return {"symbol": symbol, "status": "empty", "gaps": [], "duplicates": 0}
            ...
            for m in missing_times:
                ...
                if not pd.isna(has_next) and (has_next - m) < pd.Timedelta(hours=4) and \
                   not pd.isna(has_prev) and (m - has_prev) < pd.Timedelta(hours=4):
            ...
            if len(missing_times) > 0:
                ...
                for i in range(1, len(missing_times)):
                    if missing_times[i] == missing_times[i-1] + pd.Timedelta(minutes=1):
                        ...
                        gaps.append({"start": current_start, "end": missing_times[i-1], "count": count})
                        current_start = missing_times[i]
                        count = 1
                gaps.append({"start": current_start, "end": missing_times[-1], "count": count})
            ...
            if log_table and (gaps or duplicates_count > 0):
                table = Table(title=f"Data Health: {symbol} (Last {days}d)", title_style="bold magenta")
                ...
                for g in gaps:
                    table.add_row("GAP", g['start'].strftime('%Y-%m-%d %H:%M'), g['end'].strftime('%Y-%m-%d %H:%M'), str(g['count']))

...
class SyncDBService:
    ...
    def run_health_check(self, symbol: str, days: int = 14):
        try:
            report = DataHealthMonitor.check_integrity(symbol, days=days, log_table=False)
    ...
    def check_repair(self, symbol: str):
        ...
        for g in gaps:
            ...
            start = g['start'].strftime("%Y-%m-%dT%H:%M:%S") if hasattr(g['start'], 'strftime') else g['start']
            end = g['end'].strftime("%Y-%m-%dT%H:%M:%S") if hasattr(g['end'], 'strftime') else g['end']
    ...
    async def upload_candles(self, symbol: str, timeframe: str, gmt_offset: int, candles: List[Dict]):
        if not candles:
        ...
        with MarketDataAPI() as db:
            ...
            if not success:
                raise HTTPException(status_code=500, detail="Failed to write to InfluxDB")
    ...
    def evaluate_strategy(self, symbol: str, days: int = 3):
        try:
            with MarketDataAPI() as api:
                df = api.get_resampled_candles(symbol, interval="15m", start=f"-{days}d")
                if df.empty:
    ...
    def verify_history(self, symbol: str, gmt_offset: int, candles: List[Dict]):
        ...
        try:
            ...
            df_mt5['timestamp'] = pd.to_datetime(df_mt5['timestamp']).dt.tz_localize(None)
            df_mt5['timestamp'] = df_mt5['timestamp'] - pd.to_timedelta(gmt_offset, unit='s')
            ...
            with MarketDataAPI() as db:
                ...
                if df_db.empty:
                    logger.warning(f"Verify: DB is empty for {symbol} in range {start_time} - {end_time}. Uploading whole chunk.")
                ...
                if not missing_in_db.empty:
                    logger.info(f"Verify: Found {len(missing_in_db)} missing bars for {symbol}. Repairing...")

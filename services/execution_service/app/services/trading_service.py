...
class TradingService:
    ...
    async def broadcast_signal(self, signal: TradingSignal) -> tuple[bool, str]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                count_resp = await client.get(f"{CORE_SERVICE_URL}/positions/active/count")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if user_resp.status_code == 200:
                    ...
                    if phone:
                        side = "BUY" if event.type == 0 else "SELL" if event.type == 1 else "TRADE"
                        msg = f"✅ *TRADE OPENED*\n\n" \
                              f"Ticket: {event.ticket}\n" \
                              f"Action: {side}\n" \
                              f"Symbol: {event.symbol}\n" \
                              f"Price: {event.price}"
                        await client.post(f"{MESSAGING_SERVICE_URL}/send", json={"to": phone, "text": msg})
            except Exception as e:
    ...
    async def handle_position_closed(self, event: PositionEvent):
        ...
        logger.info(f"Server ➔ DB: UPDATE Position {event.ticket} (CLOSED) Profit: {event.profit}")
        async with httpx.AsyncClient() as client:
            try:
                await client.post(
    ...
    async def check_signals(self, symbol: str):
        ...
        try:
            ...
            df = self.market_data.get_resampled_candles(symbol, interval="15m", start="-3d")
            if df.empty:
            ...
            signal_data = Strategy.get_current_signal(df, sup_col='Sup_50', res_col='Res_50', atr_col='ATR_14')

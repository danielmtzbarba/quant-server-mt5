            ...
            try:
                ...
                for user in subscribers:
                    ...
                    for acc in user.get("broker_accounts", []):
                        ...
                        logger.info(f"Executing TRADE directly on {login}'s MT5 Engine ({user.get('name')})")
                        mt5_payload = {
                            "action": signal.action.upper(),
                            "symbol": symbol,
                        ...
                        try:
                            ...
                            if trade_resp.status_code == 200:
                                ...
                                if res_data.get("status") == "success":
                                    logger.info(f"Trade successful! Ticket: {res_data.get('ticket')}")
                                else:
                                    logger.error(f"Trade rejected by MT5: {res_data}")
                            else:
                                logger.error(f"MT5 API Failed: {trade_resp.status_code} {trade_resp.text}")
                        except Exception as mt5_err:
                            logger.error(f"Network error trying to reach MT5 Engine: {mt5_err}")

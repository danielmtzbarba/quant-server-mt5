...
class MarketVisualizer:
    ...
    def get_standard_highlights(df: pd.DataFrame, gaps: list = None) -> list:
        ...
        from .forex import score_trading_hour
        ...
        if overlays:
            ...
            if sup_cols and res_cols:
                ...
                if s_col in df.columns and r_col in df.columns:
                    # Drop NaNs for the zone to prevent rendering failures
                    zone_df = df[[s_col, r_col]].dropna()
                    if not zone_df.empty:
                        fig.add_trace(
                            go.Scatter(
                                x=zone_df.index.tolist() + zone_df.index[::-1].tolist(),
                                y=zone_df[r_col].tolist()
                                + zone_df[s_col][::-1].tolist(),
                                fill="toself",
                ...
                if col in df.columns:
                    ...
                    line_style = dict(
                        ...
                        color=(
                            ...
                            if not is_sr
                            else ("#EF5350" if "Res_" in col else "#26A69A")
                        ),
                    )

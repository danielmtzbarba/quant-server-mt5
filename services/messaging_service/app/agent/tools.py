...
EXECUTION_SERVICE_URL = get_env_var("EXECUTION_SERVICE_URL", "http://execution-service:8002")
...
async def create_watchlist(user_phone: str, ticker: str, market: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # 2. Add to watchlist
            resp = await client.post(f"{CORE_SERVICE_URL}/watchlist", params={
                "user_id": user["id"],
                "symbol": ticker.upper(),
                "market": market.upper()
            })
...
async def get_watchlist(user_phone: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(f"{CORE_SERVICE_URL}/users/{user_phone}")
            if resp.status_code != 200: return "User not found."
            user = resp.json()
...
async def delete_watchlist(user_phone: str, ticker: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            resp = await client.delete(f"{CORE_SERVICE_URL}/watchlist", params={"user_id": user["id"], "symbol": ticker.upper()})
            if resp.status_code == 200:
...
@tool
async def create_alert(user_phone: str, symbol: str, price: float, condition: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            resp = await client.post(f"{CORE_SERVICE_URL}/alerts", params={
                "user_id": user["id"], "symbol": symbol.upper(), "price": price, "condition": condition.upper()
            })
            if resp.status_code == 200:
...
async def delete_alert(user_phone: str, alert_id: int) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            resp = await client.delete(f"{CORE_SERVICE_URL}/alerts/{alert_id}", params={"user_id": user["id"]})
            if resp.status_code == 200:
...
async def get_alerts(user_phone: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            alerts = resp.json()
            if not alerts: return "No active alerts."
...
@tool
async def open_position(user_phone: str, symbol: str, action: str, volume: float) -> str:
    """Opens a new trading position. action: 'BUY' or 'SELL'."""
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.post(f"{EXECUTION_SERVICE_URL}/signal", json={
                "symbol": symbol.upper(), "action": action.upper(), "price": 0.0, "volume": volume
            })
            if resp.status_code == 200:
                return f"Trade command sent to MT5. Acknowledgment will be sent once executed."
...
async def close_position(user_phone: str, ticket: int) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.post(f"{EXECUTION_SERVICE_URL}/close_position", params={"ticket": ticket})
            if resp.status_code == 200:
...
async def get_positions(user_phone: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            positions = resp.json()
            if not positions: return "No open positions found. (Try again in 5s if you just changed something)"
...
async def verify_mt5_commands(user_phone: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if not pending: return "No pending commands in queue."

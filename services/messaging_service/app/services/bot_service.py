...
class BotService:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 404:
                    ...
                    logger.info(f"User {msg.number} not found. Triggering registration flow.")
                    # Simplified registration for now
                    await client.post(f"{CORE_SERVICE_URL}/users", params={"phone_number": msg.number, "name": "New User"})
                    response_msgs.append(msgs.text_message(msg.number, "¡Hola! 👋 Te he registrado automáticamente. ¡Ya puedes empezar!"))
            ...
            for cmd, handler in self._commands.items():
                if cmd in cleaned_text:
                    ...
                    if isinstance(result, list): response_msgs.extend(result)
                    else: response_msgs.append(result)
                    found_command = True
                    break
            
            if not found_command:
                state_input = {"messages": [HumanMessage(content=f"Mi número es {msg.number}. {msg.text}")]}
                result = await agent_executor.ainvoke(state_input, {"recursion_limit": 15})
    ...
    async def _handle_welcome(self, msg):
        ...
        return msgs.buttonReply_Message(msg, ["🗒️ listar comandos"], welcome_text, "Stonks Bot")

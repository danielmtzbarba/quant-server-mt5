        ...
        try:
            ...
            if db_resp.status_code == 200:
                print_positions_table(
                    "📊 Database Current State", db_resp.json(), source="DB"
                )
            ...
            print_positions_table(
                "📉 MT5 Current Terminals State", mt5_positions, source="MT5"
            )
            ...
            if sync_resp.status_code == 200:
                console.print(
                    "[bold green]✨ Database successfully synchronized with MT5 state.[/bold green]"
                )
                ...
                if final_resp.status_code == 200:
                    print_positions_table(
                        "✅ Final Synchronized DB State", final_resp.json(), source="DB"
                    )
            else:
                console.print(
                    f"[bold red]❌ Core Service Sync failed: {sync_resp.text}[/bold red]"
                )

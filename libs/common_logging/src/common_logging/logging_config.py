...
def setup_logging(service_name: str, tag: str = "SERVICE", color: str = "white", level=logging.INFO):
    ...
    logging.basicConfig(
        ...
        format="%(message)s",
        datefmt="[%X]",
        handlers=[RichHandler(rich_tracebacks=True, console=console, tracebacks_show_locals=True, markup=True)],
        force=True
    )

    # Silence verbose third party loggers
    for logger_name in ["urllib3", "influxdb_client", "apscheduler", "httpx", "sqlalchemy.engine"]:

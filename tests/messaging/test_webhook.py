...
async def test_whatsapp_verification_success(msg_client: AsyncClient, mocker):
    ...
    # Mock the auth token in the service
    mocker.patch("whatsapp.auth_token", "test_token")
...
async def test_incoming_trade_intent(msg_client: AsyncClient, mocker):
    ...
    mock_process = mocker.patch(
        "services.bot_service.bot_service.process_request", new_callable=AsyncMock
    )
...
async def test_send_message_api(msg_client: AsyncClient, mocker):
    ...
    # 1. Mock the WhatsApp send util
    mock_send = mocker.patch(
        "whatsapp.utils.send_message", return_value={"status": "sent"}
    )
